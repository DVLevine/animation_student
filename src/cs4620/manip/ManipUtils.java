...
public class ManipUtils {
	...
	public static float timeClosestToRay(Vector3f p, Vector3f v, Vector3f pTarget, Vector3f vTarget)
	{
		...
		//p is point of specified ray
		//v is the vector of specified ray
		//pTarget is the point of specified target ray
		//vTarget is the vector of specified target ray

		Vector3f normal = new Vector3f();
		normal.cross(v, vTarget);
		if (normal.length() < 1e-6) {
			return 0.0f;
		}

		Vector3f P21 = new Vector3f(pTarget.x, pTarget.y, pTarget.z);
		P21.sub(p);

		Vector3f M = new Vector3f(vTarget.x, vTarget.y, vTarget.z);
		M.cross(vTarget, v);
		...
		Vector3f top_1 = A;
		float top_f1 = top_1.dot(vTarget);

		Vector3f bottom = M;
		float bottomf = bottom.dot(M);

		if (bottomf == 0) {
			return 0;
		}

		float t1 = top_f1/bottomf;
	...
	public static float intersectRayPlane(Vector3f p, Vector3f v, Vector3f planeP, Vector3f planeN)
	{
		// TODO (Manipulators P1): Implement this helper method as described in the assignment
		// description and the comment above.
	        if (v.dot(planeN) == 0) { return 0; }

		planeN.scale(-1);

		Vector3f diff = new Vector3f(planeP);
		diff.sub(p);
		float t = diff.dot(planeN)/v.dot(planeN);
