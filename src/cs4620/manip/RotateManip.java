...
public class RotateManip extends Manip
{
	...
	public void dragged(Vector2f mousePosition, Vector2f mouseDelta)
	{
		...
		float maxMouseDelta = 0.25f;	// heuristically determined constant
		float maxAngleDelta = (float) (2.0f * Math.PI);
		float angleDelta = (mouseDelta.y/maxMouseDelta) * maxAngleDelta;

		Vector3f rotate = sceneNode.rotation;
		if (axisMode == PICK_X) { sceneNode.setRotation(rotate.x + angleDelta, rotate.y, rotate.z); }
		else if (axisMode == PICK_Y) { sceneNode.setRotation(rotate.x, rotate.y + angleDelta, rotate.z); }
		else if (axisMode == PICK_Z) { sceneNode.setRotation(rotate.x, rotate.y, rotate.z + angleDelta); }
	}
